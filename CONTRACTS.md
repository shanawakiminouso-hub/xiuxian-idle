...
- `sys/gongfa.js`：残篇合成（含一键合成 combineAll）、学习/升级/熟练度(挂机涨)、装备≤4、羁绊检测、隐藏功法条件监听（解锁即一次性保送残篇至 fragNeed）、自创功法 roll。
- `sys/alchemy.js`：炼丹队列(1炉，可开连炉自动续)、成功率=基础+等级+炉+火-丹毒惩罚、炸炉(材料消失概率掉灰)、变异极品(效果×1.5 icon 加★)、丹毒系统(>50 修炼-20%、>80 禁止服丹)、服用(含一键全服 useAll)、丹方解锁（探索掉落/坊市丹方栏/成就）。
- `sys/forge.js`：打造（耗材料+图纸/底材）、强化（含 enhanceTimes 连锤）、洗练(锁词条)、镶嵌、升星、套装检测、器灵孕育/养成。
...
- `sys/expedition.js`：地图解锁、派遣（选 1~3 宠 + 时长档）、特产/事件触发、隐藏图条件、一键派遣（自动最优）、连续历练（setAuto 记忆配置，结算后自动再派）。
