 ...
 * - 派遣：选地图 + 1~3 只灵宠 + 时长档（1h/4h/8h）。同时最多 3 队：
 *   第 2 队栏位=金丹1层 或 灵玉200，第 3 队栏位=化神1层 或 灵玉500。
 * - 结算规则（对齐 world.js drops 口径注释）：材料按权重抽 3/4/5 种（时长档），
 *   每种 [min,max] 取整 × 时长系数 ×1/×3.2/×6；sp 特产必掉 1 件（8h 档 2 件，不打折保底）；
 *   pill/frag 按品阶权重抽 1 档，品阶池分别随机取 pills.js 成品丹 / gongfa.js 功法残篇；
 *   eggChance / recipeChance 每次派遣独立判定（丹方→alchemy.learnRandomRecipe，残篇→gongfa.addFrag）。
 ...
 * - quickDispatch：自动为空余栏位挑空闲最强 ≤3 宠，选「地图阶位×战力系数」最高的已解锁图派 4h。
 ...
 *   DUR_OPTS → [{sec,label,kinds,mul,sp}]            时长档定义（0=1h/1=4h/2=8h）
...
(function () {
  ...
  /* ---------------- 常量 ---------------- */
  // 时长档（对齐 world.js drops 口径注释：抽 3/4/5 种材料，系数 ×1/×3.2/×6，特产 1/1/2 件）
  const DUR_OPTS = [
    { sec: 3600, label: '短途·一时', kinds: 3, mul: 1, sp: 1 },
    { sec: 14400, label: '中途·四时', kinds: 4, mul: 3.2, sp: 1 },
    { sec: 28800, label: '远游·八时', kinds: 5, mul: 6, sp: 2 },
  ];
  ...
  function settleTeam(team, isOffline) {
    ...
    if (map && map.drops) {
      ...
      // ---- sp 特产：必掉，8h 档 2 件（保底不折扣） ----
      if (map.sp) report.mat[map.sp] = (report.mat[map.sp] || 0) + opt.sp;
      // ---- 成品丹：品阶权重抽 1 档，品阶池随机 ----
      if (map.drops.pill) {
        const pe = [];
      ...
      // ---- 灵宠蛋 / 丹方：独立概率判定（受战力系数影响） ----
      if (XG.util.chance((map.drops.eggChance || 0) * factor)) report.egg = 1;
      if (XG.util.chance((map.drops.recipeChance || 0) * factor)) {
        let maxG = 1;
  ...
  // 一键派遣：空余栏位 × 空闲最强 ≤3 宠 × 期望收益最高图 × 4h 档
