...
`;
  ...
  function renderTower() {
    ...
    let h = '<div class="card"><h3 class="card-title">镇妖塔 · 第 ' + info.layer + ' 层'
      ...
      + (info.feisheng
          ? '<div class="card-sub" style="color:var(--gold-d)">已臻飞升，无限挑战，无损耗。</div>'
    ...
    if (sw) {
      h += '<div class="card"><h3 class="card-title">塔域扫荡</h3>'
        ...
        + '<div class="card-sub" style="margin-top:4px">按历史最高 ' + sw.best + ' 层折算，预估可得：灵石 ' + fmt(sw.est.lingShi) + '、修为 ' + fmt(sw.est.cult) + '，附材料/灵草/残篇若干</div></div>';
    }
    z.innerHTML = h;
  }
      ...
      dg.handler = function (ev) {
        ...
        try {
          if (act === 'dg-sub') {
            ...
            else toast('惜败于第 ' + r.layer + ' 层' + (r.penalty > 0 ? '，折损修为 ' + fmtI(r.penalty) : '') + (r.feisheng ? '，飞升之身无损分毫。' : '，且再砺剑。'), 'err');
            renderTower();
